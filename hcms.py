...
def isclose(p1,p2):
    ...
    vc_calib_ver = a_h*angle_factor
    c_calib_hor = a_w *1.7
    c_calib_ver = a_h*angle_factor
...
while True:
    ...
    if len(idxs) > 0:
        ...
        for i in idf:
            ...
            main_count=total_p
            high_str = "HIGH RISK COUNT: " + str(final_risk)
            risk = high_risk_p
            safe_str = "SAFE COUNT: " + str(safe_p)
            if(risk>=main_count/2):
                cv2.putText(FR, "Warning!!", (400, H+60),
        ...
        FR[0:H, 0:W] = frame
        frame = FR
        cv2.imshow('Human Crowd Analyzer', frame)
        cv2.waitKey(1)
    ...
    if writer is None:
        ...
        writer = cv2.VideoWriter(vname+".mp4", fourcc, 30,
                                 (frame.shape[1], frame.shape[0]), True)

    writer.write(frame)
print("Processing finished: open"+ vname+".mp4" )
writer.release()
vs.release()